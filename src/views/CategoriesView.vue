      ...
      <div class="modal-card">
        ...
        <form @submit.prevent="saveCategory" class="modal-body">
          <div class="form-group">
            ...
            <input type="text" v-model="formData.name" class="form-input" required placeholder="مثال: منصات التجارة وإدارة المتاجر" />
          </div>

          <div class="form-group">
            <label class="form-label">اسم القطاع (English) *</label>
            <input type="text" v-model="formData.name_en" class="form-input ltr-text" required placeholder="e.g. E-Commerce & Stores" />
          </div>
...
const saveCategory = async () => {
  ...
  try {
    if (isEdit.value) {
      const res = await api.put(`/dashboard/categories/${currentEditId.value}`, formData.value);
      const idx = categories.value.findIndex(c => c.id === currentEditId.value);
      if (idx !== -1) categories.value[idx] = res.data.data;
    } else {
      const res = await api.post('/dashboard/categories', formData.value);
